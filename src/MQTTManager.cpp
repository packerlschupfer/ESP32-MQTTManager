/*
 * MQTTManager.cpp - part of the ESP32-MQTTManager library
 *
...
MQTTResult<void> MQTTManager::subscribe(const char* topic, std::function<void(const String&)> callback, int qos) {
    ...
    // F22: propagate the client result instead of swallowing it with (void). A
    // false return (esp_mqtt_client_subscribe msg_id == -1: transport down,
    // outbox OOM) means the SUBSCRIBE was NOT sent; returning ok() unconditionally
    // made the firmware's failedSubscriptions bitmask / retry timer dead code and
    // logged "Subscribed" while a command topic was silently unsubscribed.
    bool subOk = mqttClient->subscribe(topicStr, [this, callback, topicStr](const std::string& payload) {
    ...
    xSemaphoreGive(mutex);
    if (!subOk) {
        MQTTM_LOG_E("Subscribe request failed for topic: %s", topic);
        return MQTTResult<void>::error(MQTTError::SUBSCRIBE_FAILED);
    }
...
MQTTResult<void> MQTTManager::subscribe(const char* topic, std::function<void(const String&, const String&)> callback, int qos) {
    ...
    std::string topicStr(topic);
    // ESP32MQTTClient uses std::string, convert to Arduino String for callback
    // F22: propagate the client result (see the payload-only overload above).
    bool subOk = mqttClient->subscribe(topicStr, [this, callback, topicStr](const std::string& topic, const std::string& payload) {
        // Convert std::string to Arduino String for callback
